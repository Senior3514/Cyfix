...
export function ReportModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Export Report" className="max-w-3xl print:max-w-none">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex gap-2">
          <button
            ...
            className={`rounded-md px-3 py-1.5 text-xs font-medium ${tab === "markdown" ? "bg-teal-500 text-graphite-950" : "bg-graphite-800 text-graphite-400"}`}
          >
            Markdown
          </button>
          <button
            onClick={() => setTab("json")}
            className={`rounded-md px-3 py-1.5 text-xs font-medium ${tab === "json" ? "bg-teal-500 text-graphite-950" : "bg-graphite-800 text-graphite-400"}`}
          >
            JSON
          </button>
        </div>
      ...
      <pre className="max-h-[50vh] overflow-auto rounded-lg border border-graphite-700 bg-graphite-950 p-4 text-[12px] leading-relaxed text-graphite-300">
        {content}
      </pre>
